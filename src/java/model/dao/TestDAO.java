...
public class TestDAO {
    ...
    public WorkerTypes create(WorkerTypes type) {
        try {
            sessionFactory.getCurrentSession().beginTransaction();
            //insert into EMPLOYEE (EMAIL, FIRST_NAME, LAST_NAME) values (?, ?, ?)
            Query query = sessionFactory.getCurrentSession().createSQLQuery("INSERT INTO [OnlineCateringDB].[dbo].[WorkerTypes] ( WorkerTypeName, PayPerDay) VALUES (:value2,:value3)");
    ...
    public WorkerTypes edit(WorkerTypes type) {
        try {
            sessionFactory.getCurrentSession().beginTransaction();
